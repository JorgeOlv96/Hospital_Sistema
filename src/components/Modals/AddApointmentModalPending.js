...
function AddAppointmentModalPending({ closeModal, isOpen, appointmentId, onDeleteAppointment }) {
    ...
    const handleProgramAppointment = async () => {
      try {
          const { fecha_programada, hora_asignada, turno, nombre_anestesiologo } = patientData;
          console.log('Datos a enviar:', { fecha_programada, hora_asignada, turno, nombre_anestesiologo });
          const response = await fetch(`http://localhost:4000/api/solicitudes/programar/${appointmentId}`, {
              method: 'PUT',
              body: JSON.stringify({ fecha_programada, hora_asignada, turno, nombre_anestesiologo }),
              headers: {
